...
public class GenericGuiHandler implements IGuiHandler {
	...
	private static Object getGuiElement(int ID, EntityPlayer player, World world, int x, int y,
			int z, boolean isClientSide) {
		switch (ID) {
			case GUI_ID_TILE_ENTITY:
				return getGuiElement(player, world, x, y, z, isClientSide);
			case 1:
				return getItemGuiElement(player, x, isClientSide);
			default:
				return null;
		}
	}

	private static Object getItemGuiElement(EntityPlayer player, int slot, boolean isClientSide) {
