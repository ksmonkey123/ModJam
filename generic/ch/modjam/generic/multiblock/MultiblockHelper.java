...
public class MultiblockHelper {
	...
	public static void constructMultiblock(World w, int x, int y, int z) {
		if (w.isRemote)
			return;
		String[] structureIDs = MultiblockRegistry.instance().getMultiblockStructureIDs();
		for (String structureID : structureIDs) {
			Multiblock structure = MultiblockRegistry.instance().getMultiblockByStructureID(
				structureID);
			MultiblockPoint[] blockPoints = structure.getValidBlockPositions(w.getBlock(x, y, z));
			for (MultiblockPoint blockPoint : blockPoints) {
				...
				if (structure.isValidStructure(w, rootX, rootY, rootZ)) {
					MultiblockHelper.registerMB(structureID, w, rootX, rootY, rootZ);
					return;
				}
			}
		}
	...
	private static void registerMB(String structureID, World w, int x, int y, int z) {
		System.out
			.println("register new MultiBlock:\n Type: " + structureID + "\nPosX: " + x + "\nPosY: " + y + "\nPosY: " + z + "\nDim:  " + w.provider.dimensionId);
