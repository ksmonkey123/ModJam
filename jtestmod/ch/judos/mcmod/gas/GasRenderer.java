...
public class GasRenderer extends TileEntitySpecialRenderer {
    ...
    protected static final int[][] offsetInbetweenBlocks = { { 1, 0, 0 },
	    { 0, 0, 1 }, { 1, 0, 1 }, { -1, 0, 1 }, // all
	    { 0, 1, 0 }, // simple y case
	...
	float vStart = vStep
		* AnimatedTextureUtils.getFramePingPong(te, 6, frameCount);
	...
	this.r.drawSprite(teA.xCoord, teA.yCoord, teA.zCoord, 1f, 1f, 0,
		vStart, 1, vStart + vStep);
	// this.r.drawSprite(teA.xCoord, teA.yCoord, teA.zCoord, 1f, 1f, 0, 0,
	// 1f, 1f);

	for (int[] off : offsetInbetweenBlocks) {
	    if (teA.connectsTo(off[0], off[1], off[2])) {
		GL11.glTranslatef(off[0] / 2f, off[1] / 2f, off[2] / 2f);
		this.r.drawSprite(teA.xCoord, teA.yCoord, teA.zCoord, 1f, 1f,
			0, vStart, 1, vStart + vStep);
		GL11.glTranslatef(-off[0] / 2f, -off[1] / 2f, -off[2] / 2f);
	    }
	}
