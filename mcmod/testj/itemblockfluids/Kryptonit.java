...
public class Kryptonit extends Block {
	...
	@Override
	public ArrayList<ItemStack> getDrops(World world, int cx, int cy, int cz,
			int metadata, int fortune) {
		
		float amountAvg = 1 + 0.33f*fortune;
		int amount = (int) Math.floor(amountAvg);
		float pro = amountAvg % 1;
		if (world.rand.nextFloat()<pro) amount++;
		...
		for (int i = 0; i < amount; i++) {
			...
			ArrayList<ItemStack> dropOre = OreDictionary.getOres(ores
					.get(index));
