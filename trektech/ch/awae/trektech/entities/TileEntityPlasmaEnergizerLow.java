 ...
 */
public class TileEntityPlasmaEnergizerLow extends ATileEntityPlasmaSystem implements IInventory,
		IHasGui {
	...
	public void customTick() {
		if (this.currentItemRemainingTime > 0) {
			...
			if (this.currentLow < PLASMA_PER_BAR * MAX_BAR && this.currentNeutral >= PLASMA_PER_TICK) {
		...
		} else if (this.currentLow < PLASMA_PER_BAR * MAX_BAR && this.currentNeutral > 0)
			this.refuel();
		else
	...
	/**
	 * determines the current burn time scaled so that the maximum burn time corresponds to
	 * <tt>h</tt>.
	 * 
	...
	public int getBurnTimeRemainingScaled(int h) {
		return this.currentItemBurnTime == 0 ? 0 : ((h * this.currentItemRemainingTime) / this.currentItemBurnTime);
	}

	/**
	 * determines the current plasma pressure level.
	 * 
	 * @param pixelPerBar the number of pixels a bar should correspond to
	 * @param maxPixel the maximum amount of pixels
	 * @param isLowPlasmaMeant <tt>true</tt> is low plasma is meant, <tt>false</tt> if neutralPlasma
	 *            is meant
	 * @return the scaled plasma pressure
	 */
	public int getPlasmaLevelScaled(int pixelPerBar, int maxPixel, boolean isLowPlasmaMeant) {
		int pixels = (isLowPlasmaMeant ? this.currentLow : this.currentNeutral) * pixelPerBar / PLASMA_PER_BAR;
		if (pixels > maxPixel)
			return maxPixel;
		return pixels;
	}
	...
	public boolean connectsToPlasmaConnection(EnumPlasmaTypes plasma, ForgeDirection direction) {
		return plasma == EnumPlasmaTypes.NEUTRAL || plasma == EnumPlasmaTypes.LOW;
	}
