 ...
 */
public class TileEntityPlasmaFurnace extends ATileEntityPlasmaSystem implements ISidedInventory,
		IHasGui, IUpgradable {
	...
	private BasicUpgrade		upgradeCache				= new BasicUpgrade(0, 0, 0);
	...
	@Override
	public int applyParticleFlow(EnumPlasmaTypes plasma, ForgeDirection direction, int particleCount) {
		switch (plasma) {
			case NEUTRAL:
	...
	public boolean onWrench(EntityPlayer player) {
		if (!player.isSneaking()) {
			if (this.worldObj.isRemote) {
				String pressure = this.currentN / (float) TileEntityPlasmaFurnace.PLASMA_PER_BAR + "";
				if (pressure.length() > 4)
					pressure = pressure.substring(0, 4);
				player.addChatMessage(new ChatComponentText(
					"Current Plasma Pressure (N): " + pressure + " bar"));
				pressure = this.currentL / (float) TileEntityPlasmaFurnace.PLASMA_PER_BAR + "";
				if (pressure.length() > 4)
					pressure = pressure.substring(0, 4);
				player.addChatMessage(new ChatComponentText(
	...
	private void setActive(boolean newState) {
		BlockGenericDualStateDirected.setActive(newState, this.worldObj, this.xCoord, this.yCoord,
			this.zCoord);
	}
	...
	private boolean canSmelt() {
		...
		if (itemstack == null)
			itemstack = TTRegistry.getAdditionalFurnaceRecipe(this.stacks[0].getItem());
		if (itemstack == null)
			if (this.makesScrap())
		...
		return result <= this.getInventoryStackLimit() && result <= this.stacks[1]
			.getMaxStackSize();
	}
	...
	private void smelt() {
		...
		this.currentN += this.getPlasmaConsumptionPerItem() * this.getPlasmaRegain();
		if (this.stacks[0] == null)
			return;
		ItemStack itemstack = FurnaceRecipes.instance().getSmeltingResult(this.stacks[0]);
		if (itemstack == null)
		...
		this.stacks[1].stackSize += this.getRealOutputCount(this.stacks[0], itemstack,
			64 - this.stacks[1].stackSize);
	...
	private int getRealOutputCount(ItemStack input, ItemStack output, int maxCount) {
		if (ListedStuff.getOreNames().contains(input.getItem().getUnlocalizedName().substring(5))) {
	...
	private int getSmeltTimePerItem() {
		return (int) (TileEntityPlasmaFurnace.BASE_BURN_TIME * (1 + this.getCache()
			.getSpeed_increase()));
	}

	private float getPlasmaRegain() {
		return TileEntityPlasmaFurnace.BASE_PLASMA_REGAIN + this.getCache().getRegain_increase();
	}

	private float getOreDoublingChange() {
		return TileEntityPlasmaFurnace.BASE_ORE_DOUBLING_CHANCE + 0.2f * this
			.getUpgradeCount(EnumUpgrade.SPECIAL);
	}
	...
	private void buildUpgradeCache() {
		...
		for (int i = 2; i < this.stacks.length; i++) {
			Item item = this.stacks[i] == null ? null : this.stacks[i].getItem();
			if (item == null || !(item instanceof ItemUpgrade))
				continue;
			upgrades[i - 2] = ((ItemUpgrade) item).getUpgradeType().getUpgrade();
		}
	 ...
	 */
	public int getPlasmaLevelScaled(int pixelPerBar, int maxPixel, boolean isLowPlasmaMeant) {
		int pixels = (isLowPlasmaMeant ? this.currentL : this.currentN) * pixelPerBar / TileEntityPlasmaFurnace.PLASMA_PER_BAR;
		if (pixels > maxPixel)
			return maxPixel;
		return pixels;
	}
