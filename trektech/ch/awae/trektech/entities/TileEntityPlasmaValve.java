 ...
 */
public class TileEntityPlasmaValve extends TileEntityPlasmaPipe implements IHasGui {
	 ...
	 * 
	 * @param plasmaType the plasma Type for this valve
	 * @param pressureThreshold the pressure to block at
	 */
	public TileEntityPlasmaValve(EnumPlasmaTypes plasmaType, float pressureThreshold) {
	...
	public int getMaxOutput(EnumPlasmaTypes plasma, ForgeDirection direction) {
		...
		return (this.currentPlasma > particleThreshold) ? particleThreshold : this.currentPlasma;
	}
	...
	public void setPressureThreshold(float threshold) {
		...
		if (this.worldObj.isRemote) {
			// Old version, could only send byte-arrays
			// this.sendNetworkCommand("set",
			// ByteBuffer.allocate(4).putFloat(this.pressureThreshold).array());
			// new Version send some serializable object (Float)
			this.sendNetworkCommand("set", this.pressureThreshold);
		}
		this.markDirty();
	}
	...
	public void onNetworkCommand(String command, Object data) {
		if (command.equals("set"))
			this.setPressureThreshold((Float) data);
	}
		...
		if (!player.isSneaking()) {
			if (this.worldObj.isRemote)
				player.addChatMessage(new ChatComponentText(
					"Current Plasma Pressure Threshold: " + this.pressureThreshold + " bar"));
			return true;
		}
		return false;
	}
