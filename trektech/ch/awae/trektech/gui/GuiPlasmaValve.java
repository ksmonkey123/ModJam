...
public class GuiPlasmaValve extends GenericGuiTEContainer {
	 ...
	 * @param tileEntity
	 */
	public GuiPlasmaValve(InventoryPlayer inventory, TileEntityPlasmaValve tileEntity) {
	...
	protected void drawGuiContainerForegroundLayer(int param1, int param2) {
		String s = StatCollector.translateToLocal(this.tileEntity.getWorld().getBlock(
		...
		this.fontRendererObj.drawString("Threshold: " + this.tileEntity.getPressureThreshold(), 50,
			43, color);
		this.fontRendererObj.drawString("Pressure : " + this.tileEntity.getPressure(), 50, 52,
			color);
	}
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		...
		this.mc.renderEngine.bindTexture(new ResourceLocation(
			TrekTech.MODID + ":textures/gui/generic.png"));
		int x = (this.width - this.xSize) / 2;
		int y = (this.height - this.ySize) / 2;
	...
	protected void actionPerformed(GuiButton button) {
		...
		switch (button.id) {
			case 0:
				this.tileEntity.setPressureThreshold(this.tileEntity.getPressureThreshold() + 1);
				break;
			case 1:
				this.tileEntity.setPressureThreshold(this.tileEntity.getPressureThreshold() - 1);
				break;
